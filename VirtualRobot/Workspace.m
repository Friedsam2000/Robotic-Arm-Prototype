    ...
    methods
        ...
        function obj = Workspace(virtualRobot)
            ...
            if exist('workspaceObject.mat', 'file')
                ...
                % Check if the joint limits are the same
                if isequal(loadedData.obj.virtualRobot.joint_limits, obj.virtualRobot.joint_limits)
                    obj.boundaryK = loadedData.obj.boundaryK;
                    obj.boundaryVertices = loadedData.obj.boundaryVertices;
                    % Load any other necessary properties
                else
                    % Joint limits have changed, recalculate the workspace
                    obj.calculateWorkspaceSerial;
                end
            else
                % File doesn't exist, calculate the workspace
                obj.calculateWorkspaceSerial;
            end
        end
        ...
        function calculateWorkspaceSerial(obj)
            ...
            % Measure time for calculation
            tic;
                
            disp("Calculating Workspace...");

            % Retrieve the joint limits of the referenced virtual robot
                ...
                for j = 1:length(q_2)
                    for k = 1:length(q_3)
                        for l = 1:length(q_4)
                            ...
                            obj.virtualRobot.setQ([q_1(i); q_2(j); q_3(k); q_4(l)]);
        
                            % Calculate and store the end-effector position
                            workspaceSamples(:, n) = obj.virtualRobot.getEndeffectorPos;
                            n = n + 1;
                        end
                    end
                end
            ...
            obj.virtualRobot.setQ(q_save);
        
            % Reduce the number of workspace points using uniquetol
            uniqueWorkspace = uniquetol(workspaceSamples', obj.workspaceTolerance, 'ByRows', true)';
